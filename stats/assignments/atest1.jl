...
html"""<p>Above we saw that variables in Julia can take single values, but they can also represent vectors or matrices of data.</p>"""
...
begin
# Make a web request to get the data via `HTTP.get` on the `url`
http_response = HTTP.get(url)
# Access the data of the response and pass
